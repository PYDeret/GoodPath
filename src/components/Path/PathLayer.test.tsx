...
describe('PathLayer', () => {
    it('renders nothing when the GTFS data is not loaded', () => {
        ...
        vi.mocked(useShortestPath).mockReturnValue({path: null, duration: null, arrivals: [], patternIds: []});
    ...
    it('draws a polyline through the stations on the path, in order', () => {
        vi.mocked(useGtfsData).mockReturnValue({data: {stations}} as ReturnType<typeof useGtfsData>);
        vi.mocked(useShortestPath).mockReturnValue({path: ['A', 'B'], duration: 300, arrivals: [0, 300], patternIds: [null, 'L1']});
